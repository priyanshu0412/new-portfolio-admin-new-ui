...
const EditBlogForm = ({ blogCategory, blogs, editBlog }) => {
    ...
    useEffect(() => {
        ...
        setValue("blogTitle", editBlog.title);
        setValue("publishDate", editBlog.date?.slice(0, 10));
        setValue("readTime", editBlog.readTime);
        setValue("blogDescription", editBlog.desc);
        setValue("blogCategory", editBlog.category?.[0]?._id);
        setValue("tags", editBlog.tags?.join(", "));
        setValue("shareLink", editBlog.shareLink);
        setValue("authorName", editBlog.authorName);
        setValue("authorGithub", editBlog.authorGithubLink);
        setValue("authorBio", editBlog.authorDesc);
        setValue("authorPortfolio", editBlog.authorPortfolioLink);
        setValue("otherSocialLink", editBlog.authorOtherProfileLink);
        setValue("isFeaturedBlog", editBlog.isFeatured);
        setEditorContent(editBlog.content);
        if (editBlog.thumbnailImg) {
            setPreview(editBlog.thumbnailImg);
        }
        setValue(
            "relatedBlogs",
            editBlog.relatedBlogs?.map((b) => b._id)
        );
