...
const EditBlogCategoryForm = ({ blogCategory }) => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const res = await FetchApi({
                url: `/blogCategory/${blogCategory._id}`,
                method: "PATCH",
                data: {
                    name: data.name,
    ...
    useEffect(() => {
        if (blogCategory) {
            setValue("name", blogCategory.name);
            setValue("description", blogCategory.description);
        }
    }, [blogCategory, setValue]);
        ...
        <>
            <div className="flex flex-col w-full min-h-screen h-full">
                ...
                <div className="flex flex-col items-start gap-y-6 w-full justify-between">
                    ...
                    <ReUseHeadingTitle
                        title={"Update Your Blog Category"}
                        desc={
                            "Update your Blog Category here and manage all your versions in one place."
                        }
                    />
                </div>
